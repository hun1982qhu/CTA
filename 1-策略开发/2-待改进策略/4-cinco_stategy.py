...
class CincoStrategy(CtaTemplate):
    ...
    def on_15min_bar(self, bar: BarData):
        ...
        boll_with = self.boll_up - self.boll_down
        ...
        if not self.pos:
            ...
            self.long_stop = self.intra_trade_high - self.trailing_long * boll_with
            self.sell(self.long_stop, abs(self.pos), stop=True)
        
        else:
            self.intra_trade_low = min(self.intra_trade_low, bar.low_price)
            self.short_stop = self.intra_trade_low + self.trailing_short * boll_with
