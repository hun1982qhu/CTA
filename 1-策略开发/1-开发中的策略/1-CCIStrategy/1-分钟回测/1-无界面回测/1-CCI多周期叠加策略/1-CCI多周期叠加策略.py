...
class CCIStrategy(CtaTemplate):
    ...
    def on_init(self):
        ...
        self.cci_cross_over = False
        self.cci_cross_below = False
    ...
    def on_stop(self):
        """"""
        self.cci_df["cci1"] = self.cci1_list[-100:]
        self.cci_df["cci2"] = self.cci2_list[-100:]

        self.cci_df.index = self.t[-100:]

        fig = plt.figure(figsize=(100, 50))
        # plt.gca().axes.get_xaxis().set_visible(False)  # 此命令可以使两幅图中处于上方的图不显示横坐标轴，而和处于下方的图共用横坐标     
        self.cci_df["cci1"].plot(lw=10, color="r")
        self.cci_df["cci2"].plot(lw=10, color="b")
        plt.axhline(100, lw=5, color='y')
        plt.axhline(0, lw=5, color='g')
        plt.axhline(-100, lw=5, color='c')
        plt.legend()
        plt.show()
    ...
    def on_Xmin_bar(self, bar: BarData):
        ...
        self.t.append(bar.datetime)

        cci1 = am.cci(self.cci_window1, True)
        self.cci1_list.append(cci1[-1])
        self.cci1 = cci1[-1]

        cci2 = am.cci(self.cci_window2, True)
        self.cci2_list.append(cci2[-1])
        self.cci2 = cci2[-1]

        self.diff = self.cci1 - self.cci2

        self.cci_cross_over = ((cci1[-2] < cci2[-2]) and (cci2[-1] > cci2[-1]))
        self.cci_cross_below = ((cci1[-2] > cci2[-2]) and (cci2[-1] < cci2[-1]))
        ...
        if self.pos == 0:
            ...
            if not self.sell_vt_orderids:
                if self.cci2 > 100 and self.diff > :
                    self.short_vt_orderids = self.short(self.short_price, self.fixed_size, True)
                    self.short_price = 0
