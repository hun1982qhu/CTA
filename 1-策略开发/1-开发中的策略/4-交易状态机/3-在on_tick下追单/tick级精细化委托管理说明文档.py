# 随着量化交易在国内金融市场越来越普及，CTA策略之间的竞争也变得越发激烈，除了体现在策略的核心交易信号方面外，
# 也同样体现在策略的实盘委托执行中。
# 大部分vn.py官方提供的CTA策略样例中，在K线回调函数on_bar内采用的都是两步操作：
# 调用cancel_all函数，全撤之前上一根K线已经挂出的委托
# 检查核心交易信号，发出新一轮的委托挂单
# 这种简单粗暴的写法，更多是出于简化策略执行中的状态机控制，帮助vn.py初学者的降低学习难度，
# 但由于较多的重复操作，在实盘中的运行效果未必能达到最佳。
# 好在策略模板CtaTemplate中委托函数（buy/sell/short/cover）可以直接返回委托号信息，
# 以及on_order/on_trade回调函数会推送委托和成交状态变化，
# 结合少量的底层代码改造，我们就可以实现更加精细的Tick级别委托挂撤单管理，
# 让策略的核心交易信号和委托执行算法更加有机地结合起来。
