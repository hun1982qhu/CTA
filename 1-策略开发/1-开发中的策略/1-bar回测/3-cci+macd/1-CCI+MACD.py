...
class CCIMACDStrategy(CtaTemplate):
    ...
    fixed_size = 1
    cci_window = 100
    macd_fastk_period = 18
    macd_slowk_period = 60
    macd_dea_period = 10
    macd_drawback_pct = 0.70
    pricetick_multilplier1 = 2
    ...
    def on_Xmin_bar(self, bar: BarData):
        ...
        self.cci_crossover_100 = (cci2 < 100 and cci1 > 100)
        # cci线下穿100线，说明市场行情上升趋势已经结束，此时应顺势卖出
        self.cci_crossbelow_100 = (cci2 > 100 and cci1 < 100)
        # cci线上穿-100线，说明市场行情开始回调，应该顺势买入
        self.cci_crossover_m100 = (cci2 < -100 and cci1 > -100)
        # cci线下穿-100线，说明市场行情进入明显下跌趋势，应该顺势卖出
        self.cci_crossbelow_m100 = (cci2 > -100 and cci1 < -100)
        ...
        if self.pos == 0:
            if cci1 > 90 or cci1 < -90:
                ...
                if not self.buy_vt_orderids:
                    if self.diff > 0 and self.dea >0 and self.macd_cross_over:
                        self.buy_vt_orderids = self.buy(self.buy_price, self.fixed_size, True)
                        self.buy_price = 0
                else:
                    for vt_orderid in self.buy_vt_orderids:
                ...
                if not self.short_vt_orderids:
                    if self.diff < 0 and self.dea < 0 and self.macd_cross_below:
                        self.short_vt_orderids = self.short(self.short_price, self.fixed_size, True)
                        self.short_price = 0
                else:
                    for vt_orderid in self.short_vt_orderids:
