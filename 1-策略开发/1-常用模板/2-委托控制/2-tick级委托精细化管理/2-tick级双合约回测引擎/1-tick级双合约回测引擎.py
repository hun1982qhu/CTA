...
# 2. 挂价发单时，维护订单在列表中排队值。根据这个TICK内成交均价和上1TICK的盘口价，3. 计算在1档盘口两边成交量，更新排队值
# 4. 每笔订单成交量不能大于盘口量
# 5. 跨交易日订单自动丢弃
# 6. 双合约回测，同时成交的两个合约按单笔结算
# 7. 保存每笔成交细节到文件
# 8. 参数扫描结果保存到文件
# 9. 每个交易日的资金曲线，保存到数据库
# 10. 增加数据线程，可以在MongoDB IO等待时撮合计算
# 11. 和实盘完全一致的成交和委托回报。回测通过的挂撤单逻辑，如果没有时序错误，基本可以直接实盘
