...
# 若价格突破上轨，触发停止买单同时取消停止卖单
# 若价格突破下轨，触发停止卖单同时取消停止买单
# 这种挂单方式在国内交易所比较少见，一般多用于外汇市场，因为货币对在短时间内会有很强的震荡，比较难判断趋势，这时候OCO的优点就彰显出来了
# 当盘整震荡的行情接近结束，而要进入到一个上升或下降的趋势时，可利用OCO挂单捕捉趋势。
# 一个例子是发生重大行情如非农或者利率决议公布，若不确定接下来的行情，可利用OCO委托进行挂单。

# OCO委托流程如下：
# 创建3个空的列表：buyOrderIDList、shortOrderIDList和ordList。
# buyOrderIDList用于缓存停止买入单的委托，分别插入委托价格，合约手数
# shortOrderIDList用于缓存停止卖出单的委托，分别插入委托价格，合约手数
