...
class PositionHolding:
    ...
    def __init__(self, contract: ContractData):
        ...
        self.long_pnl: float = 0  # 新增
        self.long_price: float = 0  # 新增
        self.long_yd: float = 0
        self.long_td: float = 0

        self.short_pos: float = 0
        self.short_pnl: float = 0  # 新增
        self.short_price: float = 0  # 新增
    ...
    def update_position(self, position: PositionData) -> None:
        ...
        if position.direction == Direction.LONG:
            ...
            self.long_pnl = position.pnl  # 新增
            self.long_price = position.price  # 新增
            self.long_yd = position.yd_volume
            self.long_td = self.long_pos - self.long_yd
            self.long_pos_frozen = position.frozen  # 新增
        else:
            self.short_pos = position.volume
            self.short_pnl = position.pnl  # 新增
            self.short_price =  position.price  # 新增
            self.short_yd = position.yd_volume
            self.short_td = self.short_pos - self.short_yd
            self.short_pos_frozen = position.frozen  # 新增

    def update_order(self, order: OrderData) -> None:
        """"""
        # if order.is_active():
        # 修改，active_orders只记录未成交和部分成交委托单
